                                                    ...
                                                    <tbody>
                                                        @foreach (App\Models\User::where('role_id', 1)->where('occupied', 0)->orderBy('name')->get() as $user)
                                                            <tr>
                                                                <td>
                                                                       <div>
                                            <input class="form-check-input me-3" type="radio" name="user_id"
                                            value="{{ $user->id }}" aria-label="...">
                                        </div>
                                ...
                                <div id="test-nl-2" role="tabpanel" class="bs-stepper-pane"
                                                    ...
                                                    <div class="d-flex align-items-center gap-3">
                                                        @foreach ($client->shifts as $shift)
                                                          <div class="form-check form-check-success">
                                                            <input class="form-check-input shiftOption" type="radio" name="shift_id" value="{{ $shift->id }}" id="flexRadioSuccess{{ $shift->id }}">
                                                            <label class="form-check-label" for="flexRadioSuccess{{ $shift->id }}">
                                                                {{ $shift->name }}
                                                            </label>
                                                          </div>
                                                          @endforeach
                                    ...
                                    <div id="assignedUserSelection" style="display: none;">
                                        ...
                                        <form id="assignedUserForm">
                                            ...
                                            <button type="button" id="removeUsers" class="btn btn-danger">Remove Selected SI</button>
                                        </form>
         ...
         <script>
            $(document).ready(function(){
                ...
                success: function(response){
                    ...
                    var siteSelectionHTML = '<h6>Select Site:</h6><div class="card"><div class="card-body"><div class="d-flex align-items-center gap-3">';
                    $.each(sites, function(index, site){
                        // siteSelectionHTML += '<li><input type="radio" name="site_id" value="' + site.id + '">' + site.name + '</li>';
                        siteSelectionHTML += '<div class="form-check form-check-success"><input class="form-check-input" type="radio" name="site_id" value="' + site.id + '"><label class="form-check-label">' + site.name + '</label></div>';
                    });
                    // $.each(sites, function(index, site){
                    //     siteSelectionHTML += '<li><input type="radio" name="site_id" value="' + site.id + '">' + site.name + '</li>';
                    // });
                    ...
                    $('#userSelection').hide(); // Hide user selection table when shift changes
                ...
                $.ajax({
                    ...
                    success: function(response){
                        ...
                        $.each(users, function(index, user){
                            userTableHTML += '<tr>';
                            userTableHTML += '<td><input type="checkbox" name="user_id[]" value="' + user.id + '"></td>';
                ...
                $.ajax({
                    type: 'GET',
                    url: '/client/{{ $client->id }}/site/' + siteId + '/shift/' + shiftId + '/assigned-users',
                    success: function(response){
                        ...
                        $.each(assignedUsers, function(index, user){
                            assignedUserTableBody.append(
                                '<tr>' +
                                '<td><input type="checkbox" name="assigned_user_id[]" value="' + user.id + '"></td>' +
                                '<td>' + user.name + '</td>' +
                                '<td>' + user.username + '</td>' +
                                '<td>' + user.email + '</td>' +
                                '<td><button type="button" class="btn btn-danger btn-sm removeUser" data-user-id="' + user.id + '">Remove</button></td>' +
                                '</tr>'
                            );
                        });
                        if (assignedUsers.length > 0) {
                        ...
                        $('#assignedUserSelection').show(); // Show assigned user table when both shift and site are selected
                    },
                    error: function(xhr){
                        console.log(xhr.responseText);
                    }
            ...
            if (selectedUsers.length === 0) {
                ...
                $.ajax({
                    type: 'POST',
                    url: '/client/{{ $client->id }}/site/' + siteId + '/shift/' + shiftId + '/add-users',
                    data: formData,
                    success: function(response){
                        alert(response.message);
                        // You can handle success behavior here, like refreshing the page or showing a success message
                        $('#siteSelection input[type=radio][name=site_id]:checked').trigger('change');
                    },
                    error: function(xhr){
                        console.log(xhr.responseText);
                    }
            ...
            $('#assignedUserTable').on('click', '.removeUser', function(){
                ...
                $.ajax({
                    type: 'POST',
                    url: '/client/{{ $client->id }}/site/' + siteId + '/shift/' + shiftId + '/remove-user',
                    data: {
                        _token: '{{ csrf_token() }}',
                        user_id: userId
                    },
            ...
            if (selectedUsers.length === 0) {
                ...
                $.ajax({
                    type: 'POST',
                    url: '/client/{{ $client->id }}/site/' + siteId + '/shift/' + shiftId + '/remove-users',
                    data: formData,
                    success: function(response){
                        alert(response.message);
                        // Reload assigned users after removing
