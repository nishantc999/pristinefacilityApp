                ...
                <div class="col-xl-8 mx-auto">
                    <div class="card">
                        ...
                        <div class="card-body p-4">
                            <form class="row g-3" method="POST" action="{{ route('clientmanagement.step_2_update', $client->id) }}"
                                enctype="multipart/form-data">
                                {{ csrf_field() }}
                                {{ method_field('PATCH') }}

                                <div id="sitesContainer">
                                    @foreach($client->sites as $siteIndex => $site)
                                        <div class="site">
                                            <label for="siteName">Site:</label>
                                            <input type="text" name="sites[{{ $siteIndex }}][name]" id="siteName" value="{{ old("sites.$siteIndex.name", $site->name) }}">
                                            <label for="shifts">Shifts:</label>
                                            <select name="sites[{{ $siteIndex }}][shifts][]" multiple class="shiftSelect">
                                                @foreach($shifts as $shift)
                                                    <option value="{{ $shift->id }}" @if(in_array($shift->id, $site->shifts->pluck('id')->toArray())) selected @endif>{{ $shift->name }}</option>
                                                @endforeach
                                            </select>
                                            <div class="areas">
                                                @foreach($site->areas as $areaIndex => $area)
                                                    <div class="area">
                                                        <label for="areaName">Area:</label>
                                                        <input type="text" name="sites[{{ $siteIndex }}][areas][{{ $areaIndex }}][name]" value="{{ old("sites.$siteIndex.areas.$areaIndex.name", $area->name) }}">
                                                        <button type="button" class="removeArea">Remove Area</button>
                                                    </div>
                                                @endforeach
                                            </div>
                                            <button type="button" class="addArea">Add Another Area</button>
                                            <button type="button" class="removeSite">Remove Site</button>
                                        </div>
                                    @endforeach
                                </div>
...
<script>
    document.getElementById('addSite').addEventListener('click', function() {
        ...
        var removeAreaBtn = document.createElement('button');
        removeAreaBtn.type = 'button';
        removeAreaBtn.classList.add('removeArea');
        removeAreaBtn.innerText = 'Remove Area';
        
        areaDiv.appendChild(areaLabel);
        areaDiv.appendChild(areaInput);
        areaDiv.appendChild(removeAreaBtn);
        ...
        var removeSiteBtn = document.createElement('button');
        removeSiteBtn.type = 'button';
        removeSiteBtn.classList.add('removeSite');
        removeSiteBtn.innerText = 'Remove Site';
        ...
        siteDiv.appendChild(removeSiteBtn);
    ...
    document.addEventListener('click', function(event) {
        ...
        if (event.target.classList.contains('removeArea')) {
            var siteDiv = event.target.closest('.site');
            var areas = siteDiv.querySelectorAll('.area');
            if (areas.length > 1) {
                var areaDiv = event.target.closest('.area');
                areaDiv.remove();
            } else {
        ...
        if (event.target.classList.contains('removeSite')) {
            var sites = document.querySelectorAll('.site');
            if (sites.length > 1) {
                var siteDiv = event.target.closest('.site');
                siteDiv.remove();
            } else {
                alert('At least one site must remain.');
            }
        }
    });
</script>
